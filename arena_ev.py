...
for i in range(1, max_trials + 1):
    ...
    # 期待値
    exp_jem_i = gem_succ * p_succ + gem_fail * p_end_fail
    exp_box_i = box_succ * p_succ
    exp_dollar_i = dollar_succ * p_succ + dollar_fail * p_end_fail
    # 期待試行回数
    expected_trials += i * (p_succ + p_end_fail)
    data.append({
        "回数": i,
        "獲得ジェム": gem_succ if p_succ>0 else gem_fail,
        "獲得BOX": box_succ if p_succ>0 else box_fail,
        "成功確率": p_succ,
        "未取得確率": p_end_fail,
        "期待ジェム": exp_jem_i,
        "期待BOX": exp_box_i,
...
cost_jem_total = entry_cost * expected_trials
cost_dollar_total = entry_cost * jem_price_dollar * expected_trials
net_jem_total = rev_jem_total - cost_jem_total
net_dollar_total = rev_dollar_total - cost_dollar_total

st.write("### ✅ 合計期待収支（純利益）")
st.write(f"期待試行回数: {expected_trials:.2f} 回")
st.write(f"総期待収入: {rev_jem_total:.2f} ジェム + {tot_box:.2f} 箱 (~{rev_jem_total - tot_jem:.2f} ジェム換算BOX)")
st.write(f"総期待収入(ドル): ${rev_dollar_total:.2f}")
