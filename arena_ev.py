...
if stop_on7:
    ...
    data=[]; tot_jem=tot_box=tot_dollar=0; exp_trials=0
    for i in range(1,max_trials+1):
        ...
        gem_fail = gem_fail_avg*max_trials; box_fail=0
    ...
    st.write(f"期待収入合計: {rev_jem_total:.2f} ジェム (~${rev_dollar_total:.2f})")
    ...
    trials = max_trials
    st.write(f"期待収入: {rev_jem*trials:.2f} ジェム + {exp_box*trials:.2f} 箱 (~${rev_dollar*trials:.2f})")
    st.write(f"総コスト: {entry_cost*trials:.2f} ジェム (~${entry_cost*jem_price_dollar*trials:.2f})")
    st.write(f"純期待利益: {(rev_jem-entry_cost)*trials:.2f} ジェム (~${(rev_dollar-entry_cost*jem_price_dollar)*trials:.2f})")
